...
import {ArrowLeftOutlined, ArrowRightOutlined, CaretLeftOutlined, CaretRightOutlined} from "@ant-design/icons";
...
const Practice = () => {
    ...
    const displayPassageOnScreen = async () => {
        ...
        if (!currPassage || isNaN(currPassage.passageId) || !currPassage.translationName) {
            console.log("displayPassageOnScreen - currPassage not ready yet - returning");
            return;
        }
        if (!currPassage.verses || currPassage.verses.length === 0) {
            setBusy({state: true, message: "Calling server to get passage " + (practiceState.currentIndex + 1) + " with passage id " +
                    currPassage.passageId + "..."});
            console.log("displayPassageOnScreen - currPassage has no verses, so calling server to get verses");
            const locMemoryPassageData: any = await memoryService.getPassage(currPassage, "SteveWarsa");
            ...
            //dispatch(stateActions.setVersesForMemoryPassage({index: currentIndex, verses: locMemoryPassageData.data.verses}));
            setPracticeState((prev: PracticeState) => {
                const locPsg = {...prev.memPassageList[practiceState.currentIndex], verses: locMemoryPassageData.data.verses};
    ...
    useEffect(() => {
        const callServer = async () => {
            ...
            // dispatch(stateActions.setMemPassageList(tempPassages));
            ...
            // console.log("Inside call server - DONE setting memPassageList.  Calling displayPassageOnScreen...");
            // displayPassageOnScreen(tempPassages);
            setBusy({state: false, message: ""});
        };
        callServer();
    }, [dispatch]);
